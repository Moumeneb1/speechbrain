...
from speechbrain.nnet.losses import kldiv_loss_moumene,kldiv_loss
...
class SLU(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        self.hparams.input_emb.cuda()
        self.hparams.slu_enc.cuda()
        self.hparams.output_emb.cuda()
        self.hparams.dec.cuda()
        self.hparams.seq_lin.cuda()
        ...
        encoder_out = self.hparams.enc(wav2vec2_out)
        ...
        with torch.no_grad():
            ...
            nlu_encoder_out = self.hparams.slu_enc(embedded_transcripts)
            e_in_nlu = self.hparams.output_emb(tokens_bos)
            h_nlu, _ = self.hparams.dec(e_in_nlu, nlu_encoder_out, transcript_tokens_lens)

            logits_nlu = self.hparams.seq_lin(h_nlu)
            p_seq_nlu = self.hparams.log_softmax2(logits_nlu)
        ...
        if (
            ...
            return p_seq,p_seq_nlu, wav_lens
        else:
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if (
            ...
            p_seq, p_seq_nlu, wav_lens = predictions
        else:
        ...
        if stage == sb.Stage.TRAIN and self.batch_count % show_results_every != 0:

            loss += kldiv_loss_moumene(p_seq,p_seq_nlu,reduction="mean",label_smoothing=0.1)
...
def dataio_prepare(hparams):
    ...
    def transcript_pipeline(transcript):
        yield transcript
        transcript_tokens_list = asr_tokenizer.encode_as_ids(transcript.upper())
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "transcript","transcript_tokens", "sig", "semantics", "tokens_bos", "tokens_eos", "tokens"],
    )
...
if __name__ == "__main__":
    ...
    hparams['kl_loss'] = KLDivLoss(reduction='batchmean')
