...
class SLU(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        logits_intent = self.hparams.seq_lin_intent(h_intent)
        ...
        if (
            ...
            p_tokens_intent, scores_intent, context_intent = self.hparams.beam_searcher_intent(encoder_out, wav_lens,inflate_tensor(torch.mean(h_intent,dim=1),self.hparams.slu_beam_size,dim=0))
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if (stage != sb.Stage.TRAIN) or (
            ...
            for i in range(len(target_intents)):
                ...
                #print(" ".join(target_slots[i][1:-1]))
                #print(" ".join(target_intents[i]) + " ".join(target_slots[i]))

                print(
                '{'+str(" ".join(predicted_intents[i]))[1:-1]
                    +', '
                    + str(" ".join(predicted_slots[i])[1:-1]+
                '}')
                )
                print(
                '{'+str(" ".join(target_intents[i]))[1:-1]
                    +', '
                    + str(" ".join(target_slots[i])[1:-1]+
                '}')
                )
                
                print("")

                target_semantics.append([*target_intents[i], *target_slots[i]])
                predicted_semantics.append([*predicted_intents[i], *predicted_slots[i]])

            
            '''with open('somefile.txt', 'a') as the_file:
                    for i in range(len(predicted_semantics)):
                        the_file.write("line"+predicted_semantics[i])'''
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            stage_stats["WER_intent"] = self.wer_metric_intents.summarize("WER")
            stage_stats["WER_slots"] = self.wer_metric_slots.summarize("WER")
            stage_stats["WER_all"] = self.wer_metric_all.summarize("WER")

        # Perform end-of-iteration things, like annealing, logging, etc.
        if stage == sb.Stage.VALID:
            old_lr, new_lr = self.hparams.lr_annealing(stage_stats["WER_all"])
            (
                old_lr_wav2vec2,
                new_lr_wav2vec2,
            ) = self.hparams.lr_annealing_wav2vec2(stage_stats["WER_all"])
            ...
            self.checkpointer.save_and_keep_only(
                meta={"WER_all": stage_stats["WER_all"]}, min_keys=["WER_all"],
            )
            ...
            with open(self.hparams.wer_file, "w") as w:
                self.wer_metric_all.write_stats(w)
