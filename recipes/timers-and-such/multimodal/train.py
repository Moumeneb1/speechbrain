...
class SLU(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        bert_token_lens = bert_token_lens.to(self.device)

        # print(bert_output.shape)

        h_bert, _ = self.hparams.dec(e_in, bert_output, bert_token_lens)
        h_wav, _ = self.hparams.dec(e_in, wav2vec2_out, wav_lens)

        # print(h_bert.shape)
        # print(h_wav.shape)

        h = torch.vstack((h_bert,h_wav))
        ...
        # print('hhhh',h.shape)
        # print('psse',p_seq.shape)

        # print(bert_token_lens.shape)
        # print(wav_lens.shape)

        encoders_lens = torch.vstack((bert_token_lens,wav_lens))
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # print(p_seq.shape)
        # print(torch.vstack((tokens_eos,tokens_eos)).shape)
        # print(tokens_eos_lens.shape)
        # print(torch.hstack((tokens_eos_lens,tokens_eos_lens)).shape)

        loss_seq = self.hparams.seq_cost(
            p_seq, torch.vstack((tokens_eos,tokens_eos)), length=torch.hstack((tokens_eos_lens,tokens_eos_lens))
        )
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            old_lr_wav2vec2, new_lr_wav2vec2,= self.hparams.lr_annealing_wav2vec2(stage_stats["SER"])
    ...
    def init_optimizers(self):
        ...
        if self.checkpointer is not None:
            ...
            self.checkpointer.add_recoverable("wav2vec2", self.optimizer)
